...
const models=quick?['hedgehog']:['crystal','hedgehog'];
...
(async()=>{
  ...
  try{
    for(const width of selectWidth)for(const model of models)for(const lighting of variants){
      const revision=model==='hedgehog'?'hedgehog':'v4',prefix=`preview-hedgehog-${revision}-${lighting}-${width}`;
      console.error(`[hedgehog] ${revision} / ${lighting} / ${width}`);
      ...
      await page.route(url=>url.pathname.endsWith('/renderer.mjs'),route=>route.fulfill({contentType:'text/javascript',body:
        fs.readFileSync(path.join(root,'renderer.mjs'),'utf8').replace('this.renderer=new THREE.WebGLRenderer','globalThis.__hedgehogScene=this;globalThis.__hedgehogThree=THREE;this.renderer=new THREE.WebGLRenderer')}));
      await page.goto(`http://127.0.0.1:8093/experiments/3d/?v=0.3.47&look=balanced&player=${model}&lighting=${lighting}`);
      ...
      await page.evaluate(async()=>{
        MazeBiters3DEngine.pause();const s=__hedgehogScene;
        const {CONCEPT_SNAKES,CONCEPT_MAZE}=await import('/experiments/3d/maze-layout.mjs');
        globalThis.__hedgehogRender=s.render.bind(s);s.render=()=>{};
        globalThis.__hedgehogState={generation:123,started:true,paused:true,speed:.5,time:1000,maze:CONCEPT_MAZE,cols:19,rows:15,
        ...
        s.reset(__hedgehogState);s.resetView();s.snakeLight.reset();
        globalThis.__hedgehogPose=(yaw=0,jaw=null)=>{
          const p=__hedgehogState.player;p.visual.x=4;p.visual.y=11;p.dir={x:Math.sin(yaw),y:Math.cos(yaw)};
          s.playerYaw=yaw;s.zoom=s.targetZoom=1.5;s.resetCamera=true;
          for(let i=0;i<3;i++)__hedgehogRender(__hedgehogState,1/60,1/60);
      ...
      const metadata=await page.evaluate(()=>({app:__mazeBiters3D.diagnostics(),renderer:__hedgehogScene.diagnostics(),
        lights:(()=>{let n=0;__hedgehogScene.scene.traverse(o=>{if(o.isLight)n++;});return n;})(),
        model:(()=>{let meshes=0,triangles=0;const materials=new Set();__hedgehogScene.player.traverse(o=>{if(o.isMesh){meshes++;triangles+=(o.geometry.index?.count??o.geometry.attributes.position.count)/3;materials.add(o.material);}});return{meshes,triangles,materials:materials.size};})()}));
      assert.equal(metadata.renderer.playerModel,model==='hedgehog'?'neon-hedgehog-v1':'crystal-biter-v4');assert.equal(metadata.app.presentation.look,'balanced');
      ...
      for(const [pose,yaw,jaw]of [['front',0,null],['rear',Math.PI,null],['profile',Math.PI/2,null],['diagonal',Math.PI/4,null],['bite',0,.30]]){
        await page.evaluate(([yaw,jaw])=>__hedgehogPose(yaw,jaw),[yaw,jaw]);
        await page.screenshot({path:path.join(root,`${prefix}-${pose}-game.png`)});
        const clip=await page.evaluate(()=>{
          const s=__hedgehogScene,p=s.player.position.clone();p.y=.7;p.project(s.camera);const b=s.renderer.domElement.getBoundingClientRect(),k=innerWidth/1920;
        ...
        if(revision==='hedgehog'){
          await page.evaluate(()=>{__hedgehogScene.player.traverse(o=>{if(o.userData.insetCore)o.visible=false;});__hedgehogScene.wallMirrors.render(__hedgehogScene.scene,__hedgehogScene.camera);});
          await page.screenshot({path:path.join(root,`${prefix}-${pose}-cores-off.png`),clip});
          await page.evaluate(()=>{__hedgehogScene.player.traverse(o=>{if(o.userData.insetCore)o.visible=true;});__hedgehogScene.wallMirrors.render(__hedgehogScene.scene,__hedgehogScene.camera);});
        }
      }
      const checks=await page.evaluate(()=>{
        const s=__hedgehogScene,p=__hedgehogState.player,props=()=>{
        ...
        };Object.assign(p,{shield:false,powered:false,dead:false,hidden:false});__hedgehogPose();const baseline=props();
        for(const state of [{shield:true},{shield:false,powered:true},{powered:false,dead:true},{dead:false,hidden:true},{hidden:false}]){
          Object.assign(p,state);__hedgehogRender(__hedgehogState,1/60,1/60);
        }
      ...
      if(!materialOnly){
        bench=await page.evaluate(()=>{
          const s=__hedgehogScene,state=__hedgehogState,p=state.player,gl=s.renderer.getContext();state.paused=false;
          const cases=[];
          for(const zoom of [1.5,2]){
            ...
            for(let i=0;i<180;i++){
              ...
              const start=performance.now();__hedgehogRender(state,1/120,1/120);gl.finish();
