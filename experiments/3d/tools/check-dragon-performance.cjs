...
// camera for the preserved v0.3.50 and the articulated-paw dragon; this is not pure GPU time.
...
if(selectedModel&&!(['dragon-v2','dragon'].includes(selectedModel)))throw new Error('Supported models: dragon-v2, dragon');
(async()=>{
 const browser=await chromium.launch({channel:'msedge',headless:true,args:['--enable-gpu','--ignore-gpu-blocklist']}),rows=[],errors=[];
 try{
  for(const width of (selectedWidth?[selectedWidth]:[1920,3840]))for(const model of (selectedModel?[selectedModel]:['dragon-v2','dragon'])){
   ...
   await page.goto(`http://127.0.0.1:8093/experiments/3d/?v=0.3.51&look=balanced&player=${model}`);
   ...
   assert.equal(modelVersion,model==='dragon-v2'?'crystal-dragon-v2':'crystal-dragon-v3','The benchmark must load the requested preserved/current model');
