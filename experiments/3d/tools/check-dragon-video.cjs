...
const modes=process.argv.includes('--before')?['v50']:process.argv.includes('--after')?['v51']:['v50','v51'];
const fixture=`
...
async function main(mode){
  const model=mode==='v50'?'dragon-v2':'dragon',expectedIdentity=mode==='v50'?'crystal-dragon-v2':'crystal-dragon-v3';
  ...
  try{
    ...
    const url=new URL(base);url.search='?v=0.3.51&look=balanced&player='+model;
    ...
    await page.evaluate(mode=>{
      ...
      globalThis.__dragonVideoVersion=mode==='v50'?'Преди · v0.3.50':'Сега · v0.3.51';
      ...
      // Baseline toes are rigid meshes; v0.3.51 exposes stable articulated tips.
    ...
    for(const metrics of motionMetrics.slice(0,4)){assert.ok(metrics.maxCadence<=1.26001);assert.ok(metrics.maxActivity>.94);}
