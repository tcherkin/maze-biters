...
const models=quick?['crystal']:['crystal-v3','crystal'];
const variants=quick?['current']:['current','contrast'];
(async()=>{
  ...
  try{
    for(const width of selectWidth)for(const model of models)for(const lighting of variants){
      const revision=model==='crystal'?'v4':'v3',prefix=`preview-biter-v4-${revision}-${lighting}-${width}`;
      console.error(`[v4] ${revision} / ${lighting} / ${width}`);
      ...
      await page.route(url=>url.pathname.endsWith('/renderer.mjs'),route=>route.fulfill({contentType:'text/javascript',body:
        fs.readFileSync(path.join(root,'renderer.mjs'),'utf8').replace('this.renderer=new THREE.WebGLRenderer','globalThis.__v4Scene=this;globalThis.__v4Three=THREE;this.renderer=new THREE.WebGLRenderer')}));
      await page.goto(`http://127.0.0.1:8093/experiments/3d/?v=0.3.46&look=balanced&player=${model}&lighting=${lighting}`);
      ...
      await page.evaluate(async()=>{
        MazeBiters3DEngine.pause();const s=__v4Scene;
        const {CONCEPT_SNAKES,CONCEPT_MAZE}=await import('/experiments/3d/maze-layout.mjs');
        globalThis.__v4Render=s.render.bind(s);s.render=()=>{};
        globalThis.__v4State={generation:123,started:true,paused:true,speed:.5,time:1000,maze:CONCEPT_MAZE,cols:19,rows:15,
        ...
        s.reset(__v4State);s.resetView();s.snakeLight.reset();
        globalThis.__v4Pose=(yaw=0,jaw=null)=>{
          const p=__v4State.player;p.visual.x=4;p.visual.y=11;p.dir={x:Math.sin(yaw),y:Math.cos(yaw)};
          s.playerYaw=yaw;s.zoom=s.targetZoom=1.5;s.resetCamera=true;
          for(let i=0;i<3;i++)__v4Render(__v4State,1/60,1/60);
      ...
      const metadata=await page.evaluate(()=>({app:__mazeBiters3D.diagnostics(),renderer:__v4Scene.diagnostics(),
        lights:(()=>{let n=0;__v4Scene.scene.traverse(o=>{if(o.isLight)n++;});return n;})(),
        model:(()=>{let meshes=0,triangles=0;const materials=new Set();__v4Scene.player.traverse(o=>{if(o.isMesh){meshes++;triangles+=(o.geometry.index?.count??o.geometry.attributes.position.count)/3;materials.add(o.material);}});return{meshes,triangles,materials:materials.size};})()}));
      assert.equal(metadata.renderer.playerModel,`crystal-biter-${revision}`);assert.equal(metadata.app.presentation.look,'balanced');
      ...
      for(const [pose,yaw,jaw]of [['front',0,null],['rear',Math.PI,null],['diagonal',Math.PI/4,null],['bite',0,.30]]){
        await page.evaluate(([yaw,jaw])=>__v4Pose(yaw,jaw),[yaw,jaw]);
        await page.screenshot({path:path.join(root,`${prefix}-${pose}-game.png`)});
        const clip=await page.evaluate(()=>{
          const s=__v4Scene,p=s.player.position.clone();p.y=.7;p.project(s.camera);const b=s.renderer.domElement.getBoundingClientRect(),k=innerWidth/1920;
        ...
        if(revision==='v4'){
          await page.evaluate(()=>{__v4Scene.player.traverse(o=>{if(o.userData.insetCore)o.visible=false;});__v4Scene.wallMirrors.render(__v4Scene.scene,__v4Scene.camera);});
          await page.screenshot({path:path.join(root,`${prefix}-${pose}-cores-off.png`),clip});
          await page.evaluate(()=>{__v4Scene.player.traverse(o=>{if(o.userData.insetCore)o.visible=true;});__v4Scene.wallMirrors.render(__v4Scene.scene,__v4Scene.camera);});
        }
      }
      const checks=await page.evaluate(()=>{
        const s=__v4Scene,p=__v4State.player,props=()=>{
        ...
        };Object.assign(p,{shield:false,powered:false,dead:false,hidden:false});__v4Pose();const baseline=props();
        for(const state of [{shield:true},{shield:false,powered:true},{powered:false,dead:true},{dead:false,hidden:true},{hidden:false}]){
          Object.assign(p,state);__v4Render(__v4State,1/60,1/60);
        }
      ...
      if(!materialOnly){
        bench=await page.evaluate(()=>{
          const s=__v4Scene,state=__v4State,p=state.player,gl=s.renderer.getContext();state.paused=false;
          const cases=[];
          for(const zoom of [1.5,2]){
            ...
            for(let i=0;i<180;i++){
              ...
              const start=performance.now();__v4Render(state,1/120,1/120);gl.finish();
