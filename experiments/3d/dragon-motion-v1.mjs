...
export class DragonMotion{
  constructor({rig,scale=2.1}){
    ...
    this.rig=rig;this.scale=scale;this.bodyLength=-Math.min(...rig.restZ)*scale;
  ...
  update(root,player,snapshot,layout,dt,wallBounds){
    ...
    if(!moved&&this.ready&&this.lastRootYaw===root.rotation.y)return;
    this.issues.clear();
    ...
    for(let i=0;i<this.rig.bones.length;i++){
      const distance=-this.rig.restZ[i]*this.scale,sample=sampleRoute(this.pieces,distance,this.samples[i]),bone=this.rig.bones[i];
      if(i===0){bone.position.set(0,0,0);bone.quaternion.identity();previousYaw=yaw;continue;}
      ...
      let worldYaw=routeYaw(this.pieces,distance,this.scratchA);
  ...
  diagnostics(){return {ready:this.ready,feasible:this.ready&&!Array.from(this.issues).some(i=>i!=='self-overlap'&&i!=='tight-bend'),
    issues:[...this.issues],selfOverlap:this.issues.has('self-overlap'),bodyLength:this.bodyLength,arcLength:this.arcLength,
    chordLength:this.chordLength,chordDeficit:this.bodyLength-this.chordLength,maxJointAngle:this.maxJointAngle,
