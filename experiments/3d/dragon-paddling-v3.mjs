...
// Distance-driven diagonal trot: world-space support, short low recovery,
// level touchdown. Route bones and gameplay still own the body and contact root.
const clamp=(x,a,b)=>Math.max(a,Math.min(b,x));
const smooth=x=>x*x*(3-2*x);
const GROUND=.002,HALF_STRIDE=.34;
export class DragonPaddling{
  constructor({rig,walls=[]}){
    ...
    this.feet=rig.paws.map(p=>{
      ...
      return {...p,upper,lower,a:upper.length(),b:lower.length(),
        ...
        offset:(p.side>0?.5:0)+(p.front?0:.5),anchor:new THREE.Vector3(),
  ...
  reset(){
    ...
    this.wasDead=false;this.wasPaused=false;this.scale=2.1;
  ...
  prepare(root,walls=this.walls){
    ...
    for(const w of this.walls)if(w.maxX>root.position.x-5&&w.minX<root.position.x+5&&
      w.maxZ>root.position.z-5&&w.minZ<root.position.z+5)this.nearWalls.push(w);
    root.updateWorldMatrix(true,true);
    for(const f of this.feet){
      f.inverse.copy(f.pivot.parent.matrixWorld).invert();
      f.neutralWorld.copy(f.neutral).applyMatrix4(f.pivot.parent.matrixWorld);
  ...
  update(root,player,snapshot,elapsed,walls=this.walls){
    ...
    // ~3.7 full strides/sec at 1x. Blocked keys cannot cause running in place.
    this.cadence=moving?this.speed*this.duty/(2*HALF_STRIDE*this.scale):0;
    const cycles=moving?distance*this.duty/(2*HALF_STRIDE*this.scale):0;
    this.totalCycles+=cycles;this.phase=(this.phase+cycles)%1;
    for(const f of this.feet){
      if(first||!f.initialized){
        f.target.copy(f.neutralWorld);f.anchor.copy(f.target);f.lastTarget.copy(f.target);
        ...
        const t=(this.phase+f.offset)%1,stance=t<this.duty;
  ...
  trajectory(f,t,activity,profile){
    ...
    let z=HALF_STRIDE*(1-2*u),lift=0;
    if(!stance){
      ...
      const m=-2*HALF_STRIDE*(1-this.duty)/this.duty;
      z=-HALF_STRIDE+2*HALF_STRIDE*smooth(u)+m*u*(1-u)*(1-2*u);
  ...
  pose(phase,activity=1,profile=0){
    ...
    for(const f of this.feet){this.trajectory(f,((phase+f.offset)%1+1)%1,activity,profile);this.place(f);}
  }
  solve(f){
    ...
    this.pole.set(f.side*.13,-.12,f.front?-1:1);
  ...
  clear(f){
    ...
    this.pawCenter.set(0,0,.054).applyMatrix4(f.wrist.matrixWorld);
    return dragonSegmentClear(this.hipWorld,this.kneeWorld,.063*this.scale,this.nearWalls)&&
      dragonSegmentClear(this.kneeWorld,this.ankleWorld,.061*this.scale,this.nearWalls)&&
      dragonSegmentClear(this.pawCenter,this.pawCenter,.113*this.scale,this.nearWalls);
  }
  place(f){
    this.solve(f);
  ...
  diagnostics(){return {kind:'grounded-trot',phase:this.phase,cycles:this.totalCycles,speed:this.speed,activity:this.activity,
    cadence:this.cadence,duty:this.duty,stride:2*HALF_STRIDE*this.scale,paused:this.wasPaused,
