// Visual-only diagonal trot. Contacts have a finite physical stroke: a foot
...
const TAU=Math.PI*2,REACH=.068,MAX_CENTER=.316,CYCLE_DISTANCE=1.65,MAX_CADENCE=7;
...
export class BiterGait{
  ...
  update(root,player,time,dt,paused=false){
    ...
    this.bobRange=.006*calm*this.activity;
    this.body.position.y=this.bobRange*.5*(1-Math.cos(this.phase*TAU));
    const pitchTarget=moving?(.003+clamp((this.speed-oldSpeed)/seconds*.00015,-.002,.003))*calm*this.activity:0;
    const rollTarget=moving?clamp(-yawDelta/seconds*.0015,-.006,.006)*calm*this.activity:0;
