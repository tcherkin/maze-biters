...
// A quiet inset emerald volume connects the nine roots. It uses their same
// basic material and draw, with vertex colors supplying depth instead of a
// new lamp, glow pass or animated sparkle. The outer silhouette is unchanged.
const innerBack=oval([0,.296,-.096],[.205,.155,.200],[0,0,0],24,16);
const backCoreCount=innerBack.index.count,coreGeometry=merge([innerBack,...cores]);
const coreColors=new Float32Array(coreGeometry.attributes.position.count*3);
const backTint=new THREE.Color(0x28b879),rootTint=new THREE.Color(0xb4cf67).multiplyScalar(1.15);
for(let i=0;i<coreGeometry.attributes.position.count;i++){
  const p=coreGeometry.attributes.position,n=coreGeometry.attributes.normal;
  // Uneven internal density gives the inset volume shape; uniform unlit
  // green would read as a flat solid filling behind the refractive surface.
  const facing=THREE.MathUtils.clamp(.48+n.getY(i)*.35-n.getZ(i)*.4-n.getX(i)*.15,0,1);
  const depth=i<backCoreCount?(.18+.24*THREE.MathUtils.smoothstep(p.getY(i),.15,.43))*(.25+.75*facing):1;
  const color=i<backCoreCount?backTint:rootTint;
  coreColors[i*3]=color.r*depth;coreColors[i*3+1]=color.g*depth;coreColors[i*3+2]=color.b*depth;
}
...
export function createCrystalBiter(){
  ...
  const glass=createSnakeFinish(0x249a66,{role:'player'});
  glass.material.transmission=.88;glass.material.thickness=.16;glass.material.envMapIntensity=.72;
  glass.material.roughness=.055;glass.material.emissiveIntensity=.13;
  glass.material.clearcoat=.55;glass.material.clearcoatRoughness=.04;
  glass.material.specularIntensity=.78;glass.material.specularColor.setHex(0xb0ead0);
  // This actor has one connected crystal mass rather than long neon tubes.
  // Keep a small head-on contribution and a restrained edge response, so
  // the back reads emerald without becoming a luminous outline. Snake
  // materials and the shared neon shader stay unchanged.
  glass.material.onBeforeCompile=shader=>{
    shader.fragmentShader=shader.fragmentShader.replace('#include <emissivemap_fragment>',`
      ...
      totalEmissiveRadiance *= .18 + emeraldEdge * .42;
    `);
  };
  glass.material.customProgramCacheKey=()=> 'crystal-biter-emerald-v3';
  glass.coreMaterial.color.setHex(0xffffff);glass.coreMaterial.vertexColors=true;
  ...
  model.userData.modelVersion='crystal-biter-v3';model.userData.finishVersion='jewelled-emerald-back-v3';
  return {model,material:lime,
    update(state){
      ...
      glass.material.emissiveIntensity=shield?.22:.13;
    },
