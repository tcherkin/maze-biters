...
const NOSE_Z=.27,TAIL_Z=NOSE_Z-4/2.1,BONE_COUNT=129;
...
function morph(g,target,name){
  const position=target.attributes.position.clone(),normal=target.attributes.normal.clone();
  position.name=normal.name=name;
  g.morphAttributes.position=[position];g.morphAttributes.normal=[normal];
  target.dispose();g.computeBoundingBox();g.computeBoundingSphere();return g;
}
function breathingChest(g){
  const target=g.clone(),p=target.attributes.position;
  for(let i=0;i<p.count;i++){
    const x=p.getX(i),y=p.getY(i),z=p.getZ(i);
    const chest=THREE.MathUtils.smoothstep(-z,.33,.58)*(1-THREE.MathUtils.smoothstep(-z,.87,1.19));
    // Expand the ribcage above the belly, keeping the neck, hips and tail fixed.
    const upper=THREE.MathUtils.smoothstep(y,.18,.53);
    p.setXYZ(i,x*(1+.020*chest*upper),y+.013*chest*upper,z);
  }
  target.computeVertexNormals();return morph(g,target,'inhale');
}
...
breathingChest(torsoShell);breathingChest(torsoCore);
...
// Upper/lower lids slide over the curved eye lens. At rest they fold just
// behind its rim; at closure they overlap along a soft, slightly lowered seam.
// The eyeball/iris itself never shrinks and nostrils are not part of the blink.
function eyelidGeometry(side){
  const center=vec(side*.246,.552,-.020),rotation=new THREE.Quaternion().setFromUnitVectors(vec(0,0,1),vec(side*.61,.13,.78).normalize());
  const build=closed=>{
    const positions=[],indices=[],v=new THREE.Vector3(),cols=32,rows=5;
    for(const upper of [true,false]){
      const offset=positions.length/3;
      for(let r=0;r<=rows;r++)for(let c=0;c<=cols;c++){
        const x=-1+2*c/cols,edge=Math.sqrt(Math.max(0,1-x*x)),u=r/rows;
        const rim=(upper?1:-1)*edge,seam=-.065*edge+(upper?-.008:.008)*edge;
        const y=lerp(rim,closed?seam:rim*(1-.004),u);
        const z=closed?.007+.071*Math.sqrt(Math.max(0,1-x*x-y*y)):-.007;
        v.set(x*.119,y*.137,z).applyQuaternion(rotation).add(center);positions.push(v.x,v.y,v.z);
      }
      for(let r=0;r<rows;r++)for(let c=0;c<cols;c++){
        const a=offset+r*(cols+1)+c,b=a+1,d=a+cols+1;indices.push(...(upper?[a,d,b,b,d,d+1]:[a,b,d,b,d+1,d]));
      }
    }
    return geometry(positions,indices);
  };
  return morph(build(false),build(true),'blink');
}
const eyelidGeometries=[eyelidGeometry(-1),eyelidGeometry(1)];
...
function roundedJoint(center,radius,stretch=1){
  const [x,y,z]=center;
  return loft([-1,-.70,0,.70,1].map(t=>({x,y,z:z+t*radius*stretch,
    rx:radius*Math.sqrt(Math.max(.008,1-t*t)),ry:radius*Math.sqrt(Math.max(.008,1-t*t))})),{sides:10});
}
function limb(side,front){
  const z=front?-.50:-1.10;
  // Broad shoulders and rounded haunches blend into the body. The lower
  // sockets and tapered muscle volumes replace long, constant-width rods.
  const shoulder=[side*(front?.180:.130),front?.360:.310,z];
  const elbow=[side*(front?.245:.225),front?.220:.185,z+(front?-.16:.16)];
  const wrist=[side*(front?.295:.275),.062,front?-.36:-1.225];
  const pawY=wrist[1]-.006,pawX=wrist[0],pawZ=wrist[2];
  const midpoint=(a,b,t,bulge)=>[a[0]+(b[0]-a[0])*t+side*bulge,a[1]+(b[1]-a[1])*t,a[2]+(b[2]-a[2])*t];
  const upperRadius=front?.112:.130,lowerRadius=front?.079:.085;
  const upper=merge([
    sweep([shoulder,midpoint(shoulder,elbow,.44,.009),elbow],
      [upperRadius*.80,upperRadius,upperRadius*.87,front?.072:.080],{sides:12,steps:9,ellipse:.91}),
    roundedJoint(shoulder,front?.095:.114,front?1.03:1.16)]);
  const lower=merge([roundedJoint(elbow,front?.081:.089),
    sweep([elbow,midpoint(elbow,wrist,.45,.006),wrist],
      [lowerRadius,lowerRadius*.98,.067,.055],{sides:12,steps:9,ellipse:.91})]);
  const pads=[roundedJoint(wrist,.061),loft([
    {z:pawZ+.120,x:pawX,y:pawY-.007,rx:.057,ry:.029},
    {z:pawZ+.073,x:pawX,y:pawY,rx:.087,ry:.050},
    {z:pawZ+.008,x:pawX,y:pawY+.006,rx:.076,ry:.060},
    {z:pawZ-.035,x:pawX,y:pawY+.013,rx:.038,ry:.038}
  ],{sides:14})],toes=[];
  // Three fleshy toes with short quartz claws, instead of four thin prongs.
  for(const digit of [-1,0,1]){
    const x=pawX+digit*.052,tipZ=pawZ+.169-Math.abs(digit)*.012;
    pads.push(sweep([[x,pawY-.010,pawZ+.082],[x,pawY-.018,pawZ+.121],[x,pawY-.024,tipZ-.021]],
      [.030,.026,.017],{sides:9,steps:6,ellipse:.81}));
    toes.push(sweep([[x,pawY-.019,tipZ-.037],[x,pawY-.018,tipZ-.015],[x,pawY-.028,tipZ]],
      [.019,.017,.0015],{sides:8,steps:5,ellipse:.85}));
  }
  const palm=merge(pads);
  const upperCore=innerShard(midpoint(shoulder,elbow,.47,0),[.030,.047,.048],0x31e080,.65,[0,0,0],front?7:10);
  const lowerCore=innerShard(midpoint(elbow,wrist,.48,0),[.022,.032,.056],0x31e080,.63,[0,0,0],front?9:12);
  const palmCore=innerShard([pawX,pawY+.002,pawZ+.053],[.048,.023,.053],0x35de87,.65,[0,0,0],front?5:8);
  const parts={upper,lower,palm,toes:merge(toes),upperCore,lowerCore,palmCore};
  for(const key of ['upper','upperCore'])parts[key].translate(-shoulder[0],-shoulder[1],-shoulder[2]);
  for(const key of ['lower','lowerCore'])parts[key].translate(-elbow[0],-elbow[1],-elbow[2]);
  for(const key of ['palm','toes','palmCore'])parts[key].translate(-wrist[0],-wrist[1],-wrist[2]);
  parts.palm.computeBoundingBox();parts.toes.computeBoundingBox();
  const soleOffset=-Math.min(parts.palm.boundingBox.min.y,parts.toes.boundingBox.min.y);
  const boneIndex=Math.round(z/TAIL_Z*(BONE_COUNT-1));
  return {...parts,z,pivot:shoulder,elbow,wrist,side,front,boneIndex,soleOffset,
    clearance:{upper:front?.117:.140,lower:.090,paw:.134},restZ:restZ[boneIndex],tip:[0,-.030,.169]};
}
const wingParts=[wing(-1),wing(1)],limbParts=[limb(-1,true),limb(1,true),limb(-1,false),limb(1,false)];
// Shoulder shells overlap rounded elbow and wrist bearings. Every piece is
// rigid and pivot-local; rotating a joint cannot pull open a seam or stretch it.
for(const part of wingParts){
  part.boneIndex=Math.round(part.z/TAIL_Z*(BONE_COUNT-1));part.restZ=restZ[part.boneIndex];
  for(const key of ['shell','web','core'])part[key].translate(0,0,-part.restZ);
}
...
export function createCrystalDragon(){
  ...
  // The same faceted, rim-lit emerald finish as the torso, with denser tint
  // and low transmission so a closed lid still covers the luminous iris.
  const eyelidMaterial=glass(0x239d58,{thickness:.10,transmission:.12,roughness:.055});
  eyelidMaterial.emissiveIntensity=.26;
  eyelidMaterial.vertexColors=false;eyelidMaterial.side=THREE.DoubleSide;
  eyelidMaterial.name='Dragon deep emerald crystal eyelids';eyelidMaterial.userData.detailedMirror=true;
  for(const material of [emerald,jade,membrane,smoke,iris,quartz])material.userData.detailedMirror=true;
  for(const material of [emerald,jade,membrane,smoke,iris,quartz,eyelidMaterial])material.userData.physicalMirror=true;
  ...
  const eyelids=eyelidGeometries.map((g,i)=>mesh(headRig,g,eyelidMaterial,`Dragon ${i?'right':'left'} curved eyelids`));
  ...
  for(const [i,part]of limbParts.entries()){
    const group=new THREE.Group();group.name=`Dragon ${i<2?'front':'rear'} ${i%2===0?'left':'right'} shoulder`;bones[part.boneIndex].add(group);
    group.position.set(part.pivot[0],part.pivot[1],part.pivot[2]-part.restZ);
    mesh(group,part.upper,emerald,'Dragon rigid upper limb');addCore(group,part.upperCore,'Dragon upper limb inset');
    const elbow=new THREE.Group();elbow.name='Dragon articulated elbow';group.add(elbow);
    elbow.position.set(part.elbow[0]-part.pivot[0],part.elbow[1]-part.pivot[1],part.elbow[2]-part.pivot[2]);
    mesh(elbow,part.lower,emerald,'Dragon rigid forelimb and elbow bearing');addCore(elbow,part.lowerCore,'Dragon forelimb inset');
    const wrist=new THREE.Group();wrist.name='Dragon articulated wrist and paw';elbow.add(wrist);
    wrist.position.set(part.wrist[0]-part.elbow[0],part.wrist[1]-part.elbow[1],part.wrist[2]-part.elbow[2]);
    mesh(wrist,part.palm,emerald,'Dragon articulated crystal palm');mesh(wrist,part.toes,quartz,'Dragon separate crystal toes');
    addCore(wrist,part.palmCore,'Dragon paw inset');
    const tip=new THREE.Object3D();tip.name='Dragon paw tip landmark';tip.position.set(...part.tip);wrist.add(tip);
    paws.push(group);attachments.push({object:group,boneIndex:part.boneIndex,restZ:part.restZ});
    paddlePaws.push({pivot:group,elbow,wrist,tip,side:part.side,front:part.front,soleOffset:part.soleOffset,clearance:part.clearance,
      restQuaternion:group.quaternion.clone(),restPosition:group.position.clone(),
      joints:[group,elbow,wrist].map(object=>({object,restQuaternion:object.quaternion.clone(),restPosition:object.position.clone()}))});
  }
  ...
  model.userData.paddleRig={paws:paddlePaws,articulated:true,model};
  model.userData.presenceRig={head:headRig,chest:[bodyShell,bodyCore],eyelids};
  model.userData.presenceVersion='breathing-blinking-v1';
  ...
  const stationRadii=restZ.map(z=>{
    ...
    if(z> -1.27&&z<-.44)radius=Math.max(radius,.43);
    return radius;
  });
  ...
  model.userData.modelVersion='crystal-dragon-v5';model.userData.finishVersion='sculpted-deep-emerald-gold-v2';
  ...
  return {model,material:emerald,
    ...
    dispose(){
      for(const material of [emerald,jade,membrane,smoke,iris,quartz,mouth,coreMaterial,eyelidMaterial])material.dispose();
      model.userData.dragonRig.skeleton.dispose();
      for(const geometry of model.userData.dragonRig.detailOwned??[])geometry.dispose();
    }
  };
}
