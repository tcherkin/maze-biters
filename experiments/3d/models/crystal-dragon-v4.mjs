...
function limb(side,front){
  const z=front?-.53:-1.01;
  const shoulder=[side*(front?.205:.194),front?.419:.355,z-.03];
  // Load-bearing legs: fore elbows fold back, hind knees fold forward.
  // Enough link length for a low, useful stride without lengthening the body.
  const elbow=[side*.275,front?.229:.195,z+(front?-.22:.20)];
  const wrist=[side*.310,.048,z+.01];
  const pawY=wrist[1]-.006,pawX=wrist[0],pawZ=wrist[2];
  const upper=merge([sweep([shoulder,elbow],[.059,.052],{sides:10,steps:4,ellipse:.88}),roundedJoint(shoulder,.063)]);
  const lower=merge([roundedJoint(elbow,.060),sweep([elbow,wrist],[.053,.045],{sides:10,steps:4,ellipse:.90})]);
  const palm=merge([roundedJoint(wrist,.047),loft([
    {z:pawZ+.105,x:pawX,y:pawY-.01,rx:.042,ry:.024},
    {z:pawZ+.071,x:pawX,y:pawY,rx:.063,ry:.039},
    {z:pawZ+.007,x:pawX,y:pawY+.006,rx:.051,ry:.044},
    {z:pawZ-.025,x:pawX,y:pawY+.014,rx:.031,ry:.030}
  ],{sides:12})]);
  const toes=[];
  for(const digit of [-1.5,-.5,.5,1.5])toes.push(sweep([
    [pawX+digit*.024,pawY-.012,pawZ+.081],
    [pawX+digit*.023,pawY-.023,pawZ+.126],
    [pawX+digit*.022,pawY-.028,pawZ+.143-Math.abs(digit)*.008]
  ],[.014,.012,.0015],{sides:7,steps:5,ellipse:.82}));
  const upperCore=tint(sweep([shoulder,elbow],[.012,.014],{sides:6,steps:4}),0x31e080,.72);
  const lowerCore=tint(sweep([elbow,wrist],[.015,.013],{sides:6,steps:4}),0x31e080,.72);
  const palmCore=innerShard([pawX,pawY+.002,pawZ+.043],[.040,.020,.050],0x35de87,.68,[0,0,0],front?5:8);
  ...
  return {...parts,z,pivot:shoulder,elbow,wrist,side,front,boneIndex,soleOffset,restZ:restZ[boneIndex],tip:[0,-.030,.139]};
}
...
export function createCrystalDragon(){
  ...
  for(const [i,part]of limbParts.entries()){
    ...
    paddlePaws.push({pivot:group,elbow,wrist,tip,side:part.side,front:part.front,soleOffset:part.soleOffset,
  ...
  model.userData.modelVersion='crystal-dragon-v4';model.userData.finishVersion='sculpted-deep-emerald-gold-v2';
