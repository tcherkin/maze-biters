// GENERATED by experiments/3d/tools/sync-engine.mjs; edit the generator and engine/*.inc.js inputs.
// Maze Biters v1.01.93.00 / f2ec88b00589a2631050bf939a0947889810c087
// Source SHA-256: 40a6225e4f6f8d871b8bd18c037a98e8c1c47ee4f0c79c6474164fb3413212cb
...
const HUD_ANIMATION_INTERVAL_MS=1000/120;
  ...
  function playerAt(x,y){
    if(player&&!player.dead&&player.lives>0&&experimentPlayerOccupies(player,x,y))
      return player;
    if(activePlayerCount>1&&player2&&!player2.dead&&player2.lives>0&&
       experimentPlayerOccupies(player2,x,y)) return player2;
    if(activePlayerCount>2&&player3&&!player3.dead&&player3.lives>0&&
       experimentPlayerOccupies(player3,x,y)) return player3;
    return null;
  }
  ...
  const MazeBrain=(()=>{
    ...
    function routeStep(entity,target,t=gameTimeNow()){
      ...
      const candidates=experimentDirections.map(d=>{
  ...
  function createPlayerState(id,previous=null){
    const start=experimentPlayerStart(id);
...
function makeSnake(x,y,len,dir) {
  ...
  function playerStartIsClear(p){return experimentPlayerStartIsClear(p);}
  ...
  function recordSnakeVisualStep(
    ...
    const currentDirections=newBody.map((p,i)=>{
      ...
      return Math.max(Math.abs(dx),Math.abs(dy))===1 ? {x:dx,y:dy} : null;
    });
  ...
  function createReverseHeadPredictionTarget(s){
    ...
    const tailOptions=experimentDirections.filter(d=>{
  ...
  function playerRepelsInhabitant(x,y,t=gameTimeNow()){
    return somePlayer(p=>!p.dead&&p.lives>0&&
      experimentPlayerOccupies(p,x,y,t)&&
      (spawnShieldIsBlocking(p,t)||isPowerMode(p,t)||experimentPlayerEscaping(p,t))
    );
  }
  ...
  function forwardOptions(s) {
    ...
    return experimentDirections.filter(d => {
      if(d.x===reverse.x && d.y===reverse.y) return false;
  ...
  function tailRetreatOptionScore(s,g,option) {
    ...
    for(let read=0;read<queue.length&&read<SEARCH_LIMIT;read++){
      ...
      for(const d of experimentDirections){
        ...
        const key=`${x},${y}`;
        if(visited.has(key)) continue;
        visited.set(key,cell.depth+experimentStepLength(d));
        recentTouches+=recentCounts.get(key)||0;
        queue.push({x,y,depth:cell.depth+experimentStepLength(d)});
      }
      if(exits>=3) junctions++;
    }
  ...
  function chooseTailRetreatDirection(s,g,options) {
    ...
    for(const option of options){
      ...
      if(score>best+1e-9){
        ...
        const optionRank=experimentDirections.indexOf(option);
        const chosenRank=experimentDirections.indexOf(chosen);
  ...
  function retreatOneStep(s,localDirection=null) {
    ...
    let options=experimentDirections.filter(d=>{
      if(d.x===reverse.x && d.y===reverse.y) return false;
      return experimentSnakeTurnAllowed(experimentSnakeHeading(s,true),d)&&
...
function drawSnakeHead(px,py,dir) {
  ...
  const experimentOpeningSnakes=[{"color":"#cf3430","body":[{"x":7,"y":2},{"x":7,"y":1},{"x":6,"y":1},{"x":5,"y":1},{"x":4,"y":1},{"x":3,"y":1},{"x":2,"y":1},{"x":1,"y":1},{"x":1,"y":2},{"x":1,"y":3},{"x":1,"y":4},{"x":1,"y":5}]},{"color":"#e4a71a","body":[{"x":17,"y":6},{"x":17,"y":5},{"x":17,"y":4},{"x":17,"y":3},{"x":17,"y":2},{"x":17,"y":1},{"x":16,"y":1},{"x":15,"y":1},{"x":14,"y":1},{"x":13,"y":1},{"x":12,"y":1}]},{"color":"#079ed1","body":[{"x":10,"y":10},{"x":10,"y":11},{"x":9,"y":11},{"x":8,"y":11},{"x":7,"y":11},{"x":7,"y":10},{"x":7,"y":9},{"x":7,"y":8},{"x":7,"y":7},{"x":7,"y":6},{"x":8,"y":6},{"x":8,"y":7}]},{"color":"#c92099","body":[{"x":16,"y":11},{"x":15,"y":11},{"x":14,"y":11},{"x":13,"y":11},{"x":12,"y":11},{"x":12,"y":10},{"x":12,"y":9},{"x":12,"y":8},{"x":13,"y":8},{"x":13,"y":7},{"x":13,"y":6},{"x":14,"y":6},{"x":15,"y":6},{"x":16,"y":6},{"x":16,"y":7},{"x":16,"y":8}]}];
